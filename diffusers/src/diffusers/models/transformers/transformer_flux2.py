...
from ..attention_processor import sam_scaled_dot_product_attention
...
class Flux2AttnProcessor:
    ...
    def __call__(
        ...
        if not vis_attn:
            ...
            # query, key, value shape: (batch, seq_len, heads, head_dim)
            # Need to transpose to (batch, heads, seq_len, head_dim) for sam_scaled_dot_product_attention
            query_t = query.transpose(1, 2)  # (batch, heads, seq_len, head_dim)
            key_t = key.transpose(1, 2)
            value_t = value.transpose(1, 2)
            
            hidden_states_t, attn_maps = sam_scaled_dot_product_attention(
                query_t, key_t, value_t, dropout_p=0.0, is_causal=False
            )
    ...
    def forward(
        ...
        if vis_attn:
            ...
            if isinstance(attn_outputs, tuple) and len(attn_outputs) == 2:
                attn_output, attn_maps = attn_outputs
            else:
    ...
    def forward(
        ...
        if vis_attn:
            ...
            if len(attention_outputs) == 2:
                ...
                if isinstance(attention_outputs[2], torch.Tensor) and attention_outputs[2].dim() >= 3:
                    # Check if third element is attn_maps (has attention map shape)
                    attn_output, context_attn_output, attn_maps = attention_outputs
                else:
                    attn_output, context_attn_output = attention_outputs[:2]
                    attn_maps = None
            else:
                attn_output, context_attn_output = attention_outputs[:2]
                attn_maps = attention_outputs[2] if len(attention_outputs) > 2 else None
        else:
            attention_outputs = self.attn(
                hidden_states=norm_hidden_states,
                encoder_hidden_states=norm_encoder_hidden_states,
        ...
        for index_block, block in enumerate(self.single_transformer_blocks):
            if torch.is_grad_enabled() and self.gradient_checkpointing:
                ...
                if vis_attn:
                    ...
                    if isinstance(block_outputs, tuple) and len(block_outputs) == 2:
                        hidden_states, attn_maps_single = block_outputs
                        # For single stream blocks, we can accumulate or use the last attn_maps
                        if attn_maps_single is not None:
                            attn_maps = attn_maps_single
                    else:
