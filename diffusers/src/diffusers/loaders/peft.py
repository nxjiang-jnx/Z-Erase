        ...
        if len(state_dict) > 0:
            ...
            try:
                if hotswap:
                    ...
                    # inject_adapter_in_model doesn't accept state_dict parameter
                    # The state_dict is set separately using set_peft_model_state_dict
                    inject_adapter_in_model(
                        lora_config, self, adapter_name=adapter_name, **peft_kwargs
                    )
                    # Note: set_peft_model_state_dict doesn't support assign parameter
                    # The meta device warnings are harmless and don't affect functionality
